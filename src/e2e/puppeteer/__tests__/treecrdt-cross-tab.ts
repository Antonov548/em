...
it('keeps four SharedWorker OPFS tabs converged through concurrent reads, edits, and reloads', async () => {
  const peers: Page[] = []
  ...
  try {
    ...
    // SharedWorker is the single database owner. Connect peer tabs one at a time so this test stresses
    // concurrent app activity rather than a distinct simultaneous cold-start lifecycle race.
    ...
    expect(runtimeConfig).toEqual(
      Array.from({ length: TAB_COUNT }, () => ({
        configuredRuntime: 'shared-worker',
